...
{\
    try { \
        ...
        if (_expected != _actual) {\
            char message [80];\
            snprintf (message, 80, "expected %ld but was: %ld", _expected, _actual);\
...
{\
    try { \
        ...
        if (fabs ((_expected)-(_actual)) > 0.001) {\
            char message [80];\
            snprintf (message, 80, "expected %lf but was: %lf", (_expected), (_actual));\
            result_.addFailure (Failure (message, name, __FILE__, __LINE__));\
        }\
    } catch(...) { \
    result_.addFailure (Failure ("Unhandled exception", name, __FILE__, __LINE__)); \
...
{\
    try { \
        ...
        if (!_actual.Equals(_expected)) { \
            char message [256];\
            snprintf (message, 256, "expected point (%f, %f, %f) but was: (%f, %f, %f)", \
...
{\
	...
	if (_expected != _actual) {\
		char message [80];\
		sprintf_ (message, 80, "expected %ld but was: %ld", _expected, _actual);\
		result_.addFailure (Failure (message, name, __FILE__, __LINE__));\
	}\
}
